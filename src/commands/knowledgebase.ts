...
module.exports = {
  ...
  async execute({
    ...
    if (
      !args.length ||
      !Object.keys(wasFromSlash ? slashKb : commandKb).includes(args[0])
    ) {
        ...
        [
          {
            ...
            value: `Type \`${prefix}${module.exports.aliases.reduce(
              ...
              wasFromSlash ? slashKb : commandKb
            ).join("\n")}\``,
            inline: false,
          },
        ],
    ...
    const article = wasFromSlash ? slashKb[args[0]] : commandKb[args[0]];
    generateAndSendEmbed(
      article,
      message,
      infoColor,
