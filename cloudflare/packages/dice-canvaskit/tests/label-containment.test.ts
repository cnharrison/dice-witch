...
import { minimumConvexPolygonClearanceV4 } from "../src/label-containment";
...
describe("CanvasKit V4 label containment", () => {
  ...
  it("uses the closest transformed label corner", () => {
    ...
    const clearance = minimumConvexPolygonClearanceV4(triangle, corners);

    expect(clearance).toBeCloseTo(2 / Math.sqrt(1.25), 12);
  });
