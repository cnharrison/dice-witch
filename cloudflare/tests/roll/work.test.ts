...
function v4Recipes() {
  const builtin =
    BUILTIN_APPEARANCE_RECIPES_R34_V3[CHAOTIC_APPEARANCE_STYLE_ID];
...
describe("RollWork Durable Object", () => {
  ...
  it("parses pending and finalized renderer-v5 snapshots", () => {
    ...
    const pendingR20 = {
      ...pendingV4,
      viewPolicy: "r20" as const,
    };
    const pendingR21 = {
      ...pendingV4,
      viewPolicy: "r21" as const,
    };
    const pendingR22 = {
      ...pendingV4,
      viewPolicy: "r22" as const,
    };
    const pendingR23 = {
      ...pendingV4,
      viewPolicy: "r23" as const,
    };
    const pendingR24 = {
      ...pendingV4,
      viewPolicy: "r24" as const,
    };
    const pendingR25 = {
      ...pendingV4,
    ...
    expect(parseRecord(JSON.stringify(pendingR20))).toEqual(pendingR20);
    expect(parseRecord(JSON.stringify(pendingR21))).toEqual(pendingR21);
    expect(parseRecord(JSON.stringify(pendingR22))).toEqual(pendingR22);
    ...
    expect(parseRecord(JSON.stringify(pendingR41))).toEqual(pendingR41);
    ...
    expect(() =>
      parseRecord(JSON.stringify({ ...pendingV3, viewPolicy: "r20" })),
    ).toThrow();
  });
  ...
  it("keeps pending V5 lifecycle identity stable through renderer finalization", async () => {
    ...
    expect(
      ...
      lifecycleSyncs.observations.map(() => "canvaskit-v4-r19"),
    );
  ...
  it("reads the interaction webhook directly when no original message id is known", async () => {
    const id = snowflakeAt(Date.now(), 92);
    const stub = work(id);
    const input = telemetryDeliveryRequest(
      id,
      "delivery-result-webhook-probe-read",
    );
    input.accounting.guildId = "100000000000000002";
    if (input.logging.context?.kind === "guild") {
      input.logging.context.guildId = "100000000000000002";
