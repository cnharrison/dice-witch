...
describe("appearanceThumbUrl", () => {
  it("builds versioned bake-route URLs", () => {
    expect(
      appearanceThumbUrl(
        "https://api.example",
        { catalogVersion: 3, rendererRevision: "canvaskit-v4-r41" },
        "material",
        "glass",
      ),
    ).toBe("https://api.example/thumbs/3-canvaskit-v4-r41/material/glass.png");
  });
});
...
describe("AppearanceThumb", () => {
  it("renders a lazy img pointing at the baked tile behind a shimmer slot", () => {
    ...
    expect(image.getAttribute("src")?.endsWith("/thumbs/3-canvaskit-v4-r41/preset/dice-witch.png")).toBe(true);
